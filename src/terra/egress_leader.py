...
class TerraEgressLeader(EgressLeader):
    def __init__(self, config: Config, multisig: SecretAccount, cli: GaiaCli):
        self._cli = cli
        self._multisig = multisig
        super().__init__(config)
        self._multisig_account_num = self._cli.get_account_details(multisig.address)[
            "value"
        ]["account_number"]
    ...
    def handle_native_swap(self, swap_event: SwapEvent) -> str:
        ...
        swap_event.data = self._cli.generate_send_tx(
            self._multisig.address, swap_event.recipient, amount
        )
    ...
    def get_completed_swap_ids(self) -> List[str]:
        ...
        for swap in Swap.objects(
            status=Status.SWAP_SIGNED, dst_network=self.native_network().value
        ):
            ...
            tx_hash = self._sign_and_send_tx(
                swap.unsigned_tx, signatures, swap.sequence
            )
        ...
        for tx_hash in self._pending_tx_hashes:
            try:
                ...
                if "not found" not in e.inner.stderr:
                    self.logger.warning(
                        f"unexpected error while querying tx {tx_hash}: {e.inner.stderr!r}"
                    )
