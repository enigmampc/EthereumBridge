...
class TerraEgressSigner(EgressSigner):
    def __init__(
        self, config: Config, signer_addr: str, multisig_addr: str, cli: GaiaCli
    ):
        self._signer = signer_addr
        super().__init__(config)
        self._multisig = multisig_addr
        self._cli = cli
        self._multisig_account_num = self._cli.get_account_details(multisig_addr)[
            "value"
        ]["account_number"]
    ...
    def get_new_submissions(self) -> Iterable[Any]:
        return Swap.objects(
            status=Status.SWAP_UNSIGNED, dst_network=self.native_network().value
        )
    ...
    def approve_submission(self, submission: Swap):
        with temp_file(submission.unsigned_tx) as tx_path:
            ...
            signature = Signatures(
                tx_id=submission.src_tx_hash, signed_tx=signed_tx, signer=self._multisig
            )
            signature.save()
            ...
            if signature_count >= self.config.signatures_threshold:
                submission.status = (
                    Status.SWAP_SIGNED
                )  # Should probably be done by the leader to avoid races...
